# docker-bitcoin-unlimited 2016-03-20 17:08:38 -0400
...
RUN `# Creating user / Adjusting user permissions`                                                                                                                                                                                                                          && \
    ...
    `# Installing bitcoind daemon`                                                                                                                                                                                                                                          && \
     ...
     echo "exec /sbin/setuser bitcoin /home/bitcoin/bitcoinUnlimited-$BITCOIN_VERSION/bin/bitcoind -server -txindex -rpcallowip=`ip route get 8.8.8.8 | awk '{print $NF; exit}'`/24 -rpcuser=$RPCUSER -rpcpassword=$RPCPASS -rpcport=$RPCPORT" >> /etc/service/bitcoind/run && \
